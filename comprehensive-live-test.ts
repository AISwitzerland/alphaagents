/**
 * COMPREHENSIVE LIVE CHAT AGENT TEST
...
class ComprehensiveLiveTest {
  ...
  async sendMessage(sessionId: string, message: string, userId: string = 'live-test-user'): Promise<any> {
    try {
  ...
  async runScenario(scenarioName: string, messages: string[], testEmail: string): Promise<TestResult> {
  ...
  async runAllTests() {
    ...
    // === TEST 7: SWISS SPECIFIC ===
    await this.runScenario(
      "Swiss Insurance Specific Test",
      [
        "UVG Unfallversicherung Offerte",
        "Hans Schweizer",
        "natashawehrli95@gmail.com",
        "+41 31 123 45 67",
        "45 Jahre, NBU Versicherung für Selbständigen"
      ],
      "natashawehrli95@gmail.com"
    );
    ...
    await this.runScenario(
      "Complex Multi-Switch Scenario",
      [
        "Guten Tag, ich interessiere mich für verschiedene Versicherungen",
        "Zuerst Krankenversicherung",
        "Sandra Komplex",
        "Ach nein, lieber Haftpflicht",
        "Eigentlich doch einen Termin",
        "Sandra Komplex", 
        "natashawehrli95@gmail.com",
        "+41 52 999 88 77",
        "Vertragsbesprechung",
        "Dienstag Vormittag wäre ideal"
      ],
      "natashawehrli95@gmail.com"
    );
